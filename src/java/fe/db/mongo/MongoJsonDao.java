...
public class MongoJsonDao {
    ...
    public JSONObject saveXml(JSONObject json) {
        ...
        try {
            ...
            if ( json.getString("xml") != null && !json.getString("xml").isEmpty() )
                mdoc.append("xmlBytes", b64.decode(json.getString("xml")) );
            else
                mdoc.append("xmlBytes", "");
            
            if ( json.getString("xmlp") != null && !json.getString("xmlp").isEmpty() )
                mdoc.append("pdfBytes", b64.decode(json.getString("xmlp")) );
            else
                mdoc.append("pdfBytes", "");
    ...
    public JSONObject getXml(String uuid, String collection) {
        ...
        try {
            ...
            if ( mdoc != null ) {
                ...
                jresp.put("xml", new String(b64.encode(mdoc.getXml())));
                jresp.put("pdf", new String(b64.encode(mdoc.getPdf())));
            }
        } catch (Exception e) {
            try { 
                jresp = new JSONObject().put("codigo", 400);
