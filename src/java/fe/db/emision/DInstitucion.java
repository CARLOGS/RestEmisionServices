...
@Entity
@Table(name = "D_INSTITUCION")
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "DInstitucion.findAll", query = "SELECT m FROM DInstitucion m"),
    @NamedQuery(name = "DInstitucion.findAllRfc", query = "SELECT m FROM DInstitucion m WHERE m.rfc = :rfc"),
    @NamedQuery(name = "DInstitucion.findAllId", query = "SELECT m FROM DInstitucion m WHERE m.id = :id")
})
public class DInstitucion implements Serializable {

    private static final long serialVersionUID = 1L;

    protected DInstitucion() {
    }

    public DInstitucion(String rfc, String nombre, String email, int estatus, Date fecha, String accesoProveedores) {
        this.rfc = rfc;
        this.nombre = nombre;
        this.email = email;
